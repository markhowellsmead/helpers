...
<head>
  <meta charset="UTF-8">
  <title>Cats</title>
  <link rel="stylesheet" href="500px-grid.css" />
  <style>
    .header {
      padding: 1rem;
    }
...
<body>
<div class="header">
  <h1>500px grid layout using CSS Flexbox</h1>
  <p>The code in this example is a simple mutation of the original JavaScript-based Angular solution coded by <a href="https://github.com/xieranmaya/">xieranmaya</a> on Github. He's written an <em>extensive</em> blog post about it <a href="https://github.com/xieranmaya/blog/issues/6">here</a>.</p>
  <p>Why is this version without JavaScript and Angular? Because if the page is delivered from a server running PHP, there's no need to over-complicate it by introducing JavaScript.</p>
  <p>Check out the source code of this PHP file to see how easy it is. The true magic lies in the CSS Flexbox code.</p>
</div>
  <div class="fav-list">
<?php
foreach ($photos as $image) {
    printf(
        '<div style="flex-grow:%1$s;flex-basis:%2$spx;">
          <i style="padding-bottom:%3$s%%"></i>
          <img src="%4$s%5$s"/>
        </div>',
        $image['width'] * 100 / $image['height'],
        $image['width'] * 360 / $image['height'],
        $image['height'] / $image['width'] * 100,
        'https://xieranmaya.github.io/images/cats/',
        $image['url']
    );
}
