...
export class LazyImage extends Component {
	...
	render() {
		...
		if (this.props.background === true) {
			...
			return (
				<figure className={className}>
					{!image.svg && <div className="o-lazyimage__preview" style={{ backgroundImage: `url('${image.pre}')` }} />}
		...
		return (
			<figure className={className}>
				...
				<noscript>
					<img {...image.attributes} className="o-lazyimage__image" src={image.org[0]} srcset={srcset} />
				</noscript>
			</figure>
		);
	}
